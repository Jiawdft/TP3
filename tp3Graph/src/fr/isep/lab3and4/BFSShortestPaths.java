...
public class BFSShortestPaths {
	...
	public boolean hadPathTo(int v){
		...
		//et on retourn la valeur du tableau marked � la position
	...
	public int distTo(int v){
		//on cherche la position du noeud v dans les tableaux 
		//et on return la valeur du tableau distance � cette position
		for (int i = 0; i < nodes.length; i++) {
			if (nodes[i] == v) {
				return distance[i];
			}
	...
	public void printSp(int v){
		...
		if (marked[vPosition] && vPosition != -1) {
			...
			while (distance[tempPosition] > 0) {
				//on stock la valeur du tableau previous � la position tempPosition
